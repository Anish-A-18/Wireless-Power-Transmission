...
#grid[500, :, 0] = fdtd.LineDetector(name="detector")
...
#x1a, y1a = np.arange(-120, 120, 1), np.arange(0, 137, 1)
#X1a, Y1a = np.meshgrid(x1a,y1a)
#lens_mask1 = Y1a <= 137 - (a1 * np.abs(X1a ** 3) + b1 * X1a ** 2 + c1 * np.abs(X1a) + d1)
...
x2, y2 = np.arange(-40, 40, 1), np.arange(0, 16, 1)
X2, Y2 = np.meshgrid(x2,y2)

# a2,b2,c2 = 0.001,-2e-7,6e-6 #grid 1000
#a2,b2,c2 = 0.01,-2e-7,6e-7 #grid 100
a2,b2,c2 = 0.01,-5e-7,2e-6 # grid 100

lens_mask2 = Y2 <=   (a2 * X2 ** 2 + b2 * np.abs(X2)  + c2 )
...
for j, col2 in enumerate(lens_mask2.T):
    for i, val2 in enumerate(np.flip(col2)):
        if val2:
            grid[ loc2+i : loc2+32-i, j+110 : j + 111, 0] = fdtd.Object(permittivity=4, name=str(i) + "," + str(j)+ "2")
            break

#grid[ loc2+16 : loc2+19, 110 :190, 0] = fdtd.Object(permittivity=4, name= "block2")

#grid[ loc2+20 : loc2+39, 124 :128, 0] = fdtd.Object(permittivity=2, name= "block3")
#grid[ loc2+20 : loc2+39, 128 :132, 0] = fdtd.Object(permittivity=3, name= "block4")
#grid[ loc2+20 : loc2+39, 132 :138, 0] = fdtd.Object(permittivity=4, name= "block5")
#grid[ loc2+20 : loc2+39, 138 :162, 0] = fdtd.Object(permittivity=4.5, name= "block6")
#grid[ loc2+20 : loc2+39, 162 :168, 0] = fdtd.Object(permittivity=4, name= "block7")
#grid[ loc2+20 : loc2+39, 168 :172, 0] = fdtd.Object(permittivity=3, name= "block8")
#grid[ loc2+20 : loc2+39, 172 :176, 0] = fdtd.Object(permittivity=2, name= "block9")



#grid[500,120:180, 0] = fdtd.LineDetector(name="detector")
grid[100:550,50:250, 0] = fdtd.BlockDetector(name="detector")
# x boundaries
